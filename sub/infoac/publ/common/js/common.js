...
$(function(){
	...
	//특수문자 입력금지
	//ie에서 한글입력시 문제있음
	/*$('.exceptSpecialChar').keypress(function(event) {

		if (event.keyCode == 47 || event.keyCode == 92 || event.keyCode == 58 || event.keyCode == 42 || event.keyCode == 63 || event.keyCode == 34 || event.keyCode == 60 || event.keyCode == 62 || event.keyCode == 124 ){
			event.preventDefault();
		}
	 }).keyup(function(){
		if( $(this).val() != null && $(this).val() != '' ) {
...
//		html += '<select id="' + sel_id + '" class="selectA sel_file_box">';
		html += '<select id="' + sel_id + '" class="w100">';
		...
		//여능원에 맞게 수정함
		html += '</select>';
		html += '<ol id="' + file_box_id + '" class="input_file_wrap">';
		html += '	<li>';
		html += '		<label class="f_style">';
		html += '		<input type="file" name="' + tag_n + '" />파일 선택';
		html += '		</label>';
		html += '		<span class="file_name">파일이름</span>';
		html += '	</li>';
		html += '</ol>';
	}
	...
	$('#' + sel_id).on({
		'change' : function(e){
			...
			for(var i = 0; i < n; i++){
				txt += '	<li>';
				txt += '		<label class="f_style">';
				txt += '		<input type="file" name="' + tag_n + '" />파일 선택';
				txt += '		</label>';
				txt += '		<span class="file_name">파일이름</span>';
				txt += '	</li>';
			}
...
function goFaceBook(msg,url){
	...
	var a = window.open(href,'facebook','width=400,height=500');
	snsTraceAjax(msg,url,"FB");
	if(a){
		a.focus();
	}
}
...
function goTwitter(msg,url){
	...
	var a = window.open(href,'twitter','width=400,height=500');
	snsTraceAjax(msg,url,"TW");
	if(a){
		a.focus();
	}
}
...
function goNaverBlog(msg,url) {
    var href = "https://share.naver.com/web/shareView.nhn?url=" + encodeURI(encodeURIComponent(url)) + "&title=" + encodeURIComponent(msg);
    var a = window.open(href,'naverblog','width=400,height=500');
    snsTraceAjax(msg,url,"NB");
	if(a){
		a.focus();
	}
  }
...
function goKakao(msg,url) {
	Kakao.Story.share({
	  url: url,
	  text: msg
	});
	snsTraceAjax(msg,url,"KS");
}

function snsTraceAjax(msg, url, SNS_CODE){
	$.ajax({
		type		 : "POST",
		dataType 	 : 'json',
		url			 : "/common/inc/snsTraceAjax.do",
		data 		 : {
			sns_code : SNS_CODE,
			menu_nm : msg,
			refrn_url : url
...
/* 사업자번호 포맷 변경 : onkeyup="bsnmNoFomatter(this);" 형식으로 사용 */
function bsnmNoFomatter(obj){
	var num = obj.value;
    num = num.replace(/[^0-9]/g, '');
...
function fnHtmlTagFilterRestore(src) {
...
function fnEvaluationPopup(){
	var x = 1000, y = 700;
	var popupX = (window.screen.width / 2) - (x / 2); // 만들 팝업창 좌우 크기의 1/2 만큼 보정값으로 빼주었음
//	var popupY= (window.screen.height /2) - (y / 2); // 만들 팝업창 상하 크기의 1/2 만큼 보정값으로 빼주었음
	window.open('/evaluation/evaluationStart.do', 'popup', 'width='+x+', height='+y+', left='+popupX+', top=50, scrollbars=yes');
}
