$(function(){
	...
	$(window).load(function(){
		...
		lmMHeight = 0,
		$footer = $(".footer"),
		$commonLeft = $(".common_left"),
		$commonRight = $(".common_right");
		
		$(window).resize(function(){
			/* mGnb_control */
			windowW = window.innerWidth;
			windowH = $(window).height();
			cRightH = $commonRight.innerHeight();
			if(windowW>=991){
				if(mGnbReady){
					mGnbClose();
					mGnbReady = false;
			...
			windowMH = windowH - ($header.height() + $footer.height());
			lmMHeight = windowMH>cRightH ? windowMH : cRightH;
			// console.log("windowMH : " + windowMH + ", cRightH : " + cRightH + ", lmMHeight : " + lmMHeight);
			$commonLeft.css({"min-height":lmMHeight});
		});
